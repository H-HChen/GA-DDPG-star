...
class ValidGraspChecker():
    ...
    def extract_grasp(self, grasp, placed_uid, drawback_distance=0., visual=False, filter_elbow=True):
        '''
        ...
        table_box = np.array(p.getAABB(self.table_id))
        valid_grasp = []
        valid_grasp_index = []
        ...
        for i in range(grasp.shape[0]):
            ...
            if len(p.getClosestPoints(self.robot, self.table_id, 0.0001)):
                continue
